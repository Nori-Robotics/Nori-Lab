...
### [NEW 2026-07-01] Multi-camera video — 3 robot cameras, only 1 shown today
The robot has **three cameras** (`left_wrist`, `right_wrist`, `overhead`/front) but teleop currently streams only one. Both the fix and the "which device is which" problem are solved patterns in `NoriTeleopReference` — port them into the live `rpi5/media` WebRTC path.

- **Current state:** `rpi5/media/webrtc_robot.py` hardcodes a single source (`--device /dev/video0`, one `v4l2src` branch into `webrtcbin`). The negotiation is already N-camera-ready — `_ensure_offer()` sets *all* transceivers `SENDONLY` generically, and the audio track already shows the "append another branch into `sendrecv.`" pattern — so only the pipeline string is single-camera, not the machinery.

- ✅ **Camera identification — by-path, not by-id (decided + BUILT 2026-07-01).** Cheap identical UVC cameras usually share/blank their USB **serial**, so `/dev/v4l/by-id` collides across three of the same model. `/dev/v4l/by-path` keys on the **physical USB port/topology** instead → three ports = three stable names. **`rpi5/media/configure_cameras.py`** (ported from `NoriTeleopReference/.../rpi4/configure_cameras.py`) enumerates `/dev/v4l/by-path/*usb*video-index0`, has the operator **unplug LEFT WRIST then RIGHT WRIST** (deduced by set-difference), leaves the last-remaining device as **overhead**, and writes a `role → by-path` `cameras.json` (clean WebRTC schema, no ZMQ port). Adds `--verify` (boot health-check, non-zero → re-run) + `--list`; off-hardware logic test in `test_configure_cameras.py`. (The reference's newer approach never made it into the live `NoriTeleop/rpi4/image_server.py`, which still uses fragile integer indices `head=0, right_wrist=2` — this supersedes that.)

- **Persistence — manufacturing vs first-boot.** `by-path` is **stable across reboots** but **not** across re-cabling to a different port. So: if internal camera cabling is fixed at assembly, **bake `cameras.json` at the factory** (`configure_cameras.py` becomes the factory provisioning tool) and no user ever touches it. If per-unit assembly makes paths vary, fall back to a **one-time first-boot unplug/replug** run of the same tool. Either way it's the same script. (Same spirit as R16's "deterministic udev naming keyed on port path/serial" for the motor buses.)

- **Pipeline change:** `webrtc_robot.py` reads `cameras.json` and appends **one video branch per camera** into the existing `webrtcbin` (mirroring the audio branch's `... ! sendrecv.`). No transceiver-loop change needed.

- **Operator routing:** the robot must **announce the `mid → role` map** so the operator attaches each track to the right pane instead of guessing by order (m-line order is not a stable role signal once audio m-lines interleave). This is a small additive field on the media-bridge message layer (call/media signaling, **not** the daemon's `nori-protocol` — the daemon never sees video). Laptop side routes by `ev.transceiver.mid` → role.

- **⚠️ CPU/thermal caveat (blocking for hardware validation):** the R11 spike measured **one** software-H.264 stream (26% of a core, `loop_hz` ~50). **Three** concurrent SW encodes on the Pi 5 (no HW encoder) is unproven — re-run `encoder_spike.sh` + `measure_loop_hz.py` with all three before committing, and expect the two wrist feeds to run at **lower res/fps** than the primary. Budget the extra USB bandwidth against R16.
