...
// `robot`), so pasted/generated scripts call e.g. `await robot.home("left")`. Each logs a `[lib]`
// marker so you can observe which primitives scripts actually use.
//
// Poses are ABSOLUTE normalized targets ([-100,100]) and go through robot.moveTo (client-slewed +
...
export function installPrimitives(robot: ScriptRobot): void {
  const r = robot as ScriptRobot & Primitives;

  r.home = (side) => {
    robot.log(`[lib] home(${side})`);
    return robot.moveTo(side, HOME_POSE);
  };

  r.stow = (side) => {
    robot.log(`[lib] stow(${side})`);
    return robot.moveTo(side, STOW_POSE);
  };

  r.gripSequence = async (side) => {
    robot.log(`[lib] gripSequence(${side})`);
    await robot.grip(side, "open");
    await robot.wait(300);
    await robot.grip(side, "close");
  };

  r.wave = async (side, times = 3) => {
    robot.log(`[lib] wave(${side}, ${times})`);
    for (let i = 0; i < times; i++) {
      await robot.joint(side, { wrist_flex: 0.4 }, 400);
      await robot.joint(side, { wrist_flex: -0.4 }, 400);
    }
  };
}
