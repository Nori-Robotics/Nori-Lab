...
**Mechanical constants (HW team answers, 2026-07-02):** 28.455 mm travel per motor revolution (lead screw, direct drive; both lifts identical) — now the daemon default. Full travel **950 mm (tall variant) / 650 mm (short variant)** ≈ 33.4 / 22.8 motor revs — set `NORI_LIFT_TRAVEL_MM` per unit (deploy env; enables soft limits + the R-X.10 sanity check). Still open (need a unit): **max safe velocity** (default stays a conservative 4000 raw ≈ 28 mm/s ≈ full tall travel in ~34 s) and **direction** (`NORI_LIFT_SIGN=-1` if +rotation turns out to drive the lift down).

### 5.5.1 Components & tasks
- [x] **Multi-turn tracker** in the daemon *(2026-07-02: `lift.hpp`/`lift.cpp` `LiftAxis` + per-tick `read_lift_ticks()` in `main.cpp`; wrap detection, dropped-read desync tripwire (R-X.10), travel sanity check. `NORI_LIFT_MM_PER_REV` now defaults to the HW-confirmed 28.455 — heights are real millimeters; `NORI_LIFT_SIGN` added for the pending direction test)*.
...
### 5.5.3 Notes / gating
- **Hardware-gated** — needs a unit (no endstop hardware required anymore — homing is stall-based against the frame); batch validation with the other hardware-gated work. On-unit checklist: direction test (`NORI_LIFT_SIGN`), max safe velocity (`NORI_LIFT_MAX_VEL`), stall-homing thresholds, P gain feel.
